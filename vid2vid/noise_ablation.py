...
VIDEOS = [
    ("./source_video/soccerball.mp4", "a soccer ball is sitting in the grass next to a tree, in flat 2d anime"),
    ("./source_video/dog-agility.mp4", "a dog running through a field of poles, in flat 2d anime"),
    ("./source_video/gold-fish.mp4", "goldfish in aquarium, in flat 2d anime"),
    ("./source_video/tennis.mp4", "a man holding a tennis racket on a tennis court, in flat 2d anime"),
    ("./source_video/walking.mp4", "a man taking a selfie with a woman on a street, in flat 2d anime"),
    ("./source_video/tennis-vest.mp4", "a man holding a tennis racket on a tennis court, in flat 2d anime"),
    ("./source_video/flamingo.mp4", "a group of flamingos standing in the water, in flat 2d anime"),
    ("./source_video/sheep.mp4", "a group of sheep and a baby lamb in a field, in flat 2d anime"),
    ("./source_video/guitar-violin.mp4", "two men playing violin and guitar in a room, in flat 2d anime"),
]
...
def main():
    ...
    for video_path, prompt in VIDEOS:
        ...
        for noise in NOISE_LEVELS:
            ...
            # Define output path
            output_name = f"{video_name}_noise{noise}.mp4"
            ...
            from metrics.semantic_match import compute_clip_score
            clip_avg, clip_max = compute_clip_score(output_path, prompt, frame_interval=10, device="cuda")
            print(f"✓")
    ...
    for noise in sorted(results.keys()):
        ...
        clip_avgs = [r['clip_avg'] for r in results[noise].values() 
                     if r['clip_avg'] is not None]
        clip_maxs = [r['clip_max'] for r in results[noise].values() 
                     if r['clip_max'] is not None]
        
        # Compute averages
        avg_warp = sum(warp_errors) / len(warp_errors) if warp_errors else None
        avg_clip_avg = sum(clip_avgs) / len(clip_avgs) if clip_avgs else None
        avg_clip_max = sum(clip_maxs) / len(clip_maxs) if clip_maxs else None
        
        summary_table.append({
            'noise': noise,
            'avg_warp_error': avg_warp,
            'avg_clip_avg': avg_clip_avg,
            'avg_clip_max': avg_clip_max,
            'num_success': len(warp_errors)
        })
    
    # Print summary table
    print()
    print(f"{'Noise':<10} {'Avg Warp Error':<20} {'Avg CLIP (avg)':<20} {'Avg CLIP (max)':<20} {'Success':<10}")
    print("-" * 80)
    
    for row in summary_table:
        noise = row['noise']
        warp = f"{row['avg_warp_error']:.6f}" if row['avg_warp_error'] is not None else "N/A"
        clip_a = f"{row['avg_clip_avg']:.6f}" if row['avg_clip_avg'] is not None else "N/A"
        clip_m = f"{row['avg_clip_max']:.6f}" if row['avg_clip_max'] is not None else "N/A"
    ...
    if valid_rows:
        ...
        print(f"   Avg CLIP Score: {best_warp['avg_clip_avg']:.6f}")
    
    valid_rows_clip = [r for r in summary_table if r['avg_clip_avg'] is not None]
    if valid_rows_clip:
        best_clip = max(valid_rows_clip, key=lambda x: x['avg_clip_avg'])
        print(f"\n✨ Highest CLIP Score (best semantic alignment):")
